library("dplyr")
undisc_arff <- foreign::read.arff("~/projects/roughpression/pipeline/results/oral/oral.arff")
disc_arff <- foreign::read.arff("~/projects/roughpression/pipeline/results/oral/discretised.arff")
disc_table <- as.data.frame(disc_arff)

rules <- read.csv("~/projects/roughpression/pipeline/results/oral/rule-support.csv", header = FALSE, stringsAsFactors = FALSE)
...
accuracy(roughset, "depressed")
quality(roughset, "depressed", sum(disc_table_reduct$class == "depressed"))
