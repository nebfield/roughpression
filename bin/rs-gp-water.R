...
GP_collapsed <- phyloseq::subset_samples(GPfr, SampleType == "Freshwater" | SampleType == "Freshwater (creek)")
GP_collapsed <- t(GP_collapsed)
GP_y <- phyloseq::sample_data(GP_collapsed)$SampleType
otu <- data.frame(phyloseq::otu_table(GP_collapsed))
ra_annot <- cbind(data.frame(phyloseq::otu_table(GP_collapsed), Class = GP_y, check.names = FALSE))
...
accuracy(roughset, "Freshwater")
quality(roughset, "Freshwater", sum(discretised$Class == "Ocean"))
accuracy(roughset, "Freshwater (creek)")
quality(roughset, "Freshwater (creek)", sum(discretised$Class == "Freshwater (creek)"))
...
print(phyloseq::tax_table(GP_collapsed)[interesting_bug, ])
# Cenarchaeaceae
